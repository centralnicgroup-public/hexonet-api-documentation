...
## DESCRIPTION
Order a new SSL certificate as a replacement for an existing one expiring soon.
...
---- | ---- | ---- | ----
COMMAND | 1 | `RenewSSLCert` | COMMAND
...
SSLCERTID | 0 | SSL certificate ID | TEXT
...
SERVERSOFTWARE | 0 | Web server software; required for some products; possible values:<br>`APACHESSL`<br>`APACHESSLEAY`<br>`APACHE2`<br>`IIS`<br>`OTHER` | TEXT or NULL
...
CSRCONTACT0EMAIL | 0 | CSR contact email address | TEXT or NULL
...
CREATECSR | 0 | If set to `1` a new CSR will be created automatically; otherwise the old CSR will be re-used; only relevant if no CSR is provided | `0`, `1` or NULL
