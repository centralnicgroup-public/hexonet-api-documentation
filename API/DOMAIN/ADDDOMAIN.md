...
X-EU-REGISTRANT-CITIZENSHIP | 0 | Citizenship of the registrant. Only relevant for natural persons with an European citizenship residing outside of the EU | `at`, `be`, `bg`, `cz`, `cy`, `de`, `dk`, `es`, `ee`, `fi`, `fr`, `gr`, `hu`, `ie`, `it`, `li`, `lt`, `lu`, `lv`, `mt`, `nl`, `pl`, `pt`, `ro`, `se`, `sk`, `si` or `hr`
X-EU-REGISTRANT-LANG | 0 | Language of the .EU Domain registrant; possible values:<br>`cs`<br>`da`<br>`de`<br>`el`<br>`en`<br>`es`<br>`et`<br>`fi`<br>`fr`<br>`hu`<br>`it`<br>`li`<br>`lt`<br>`lv`<br>`mt`<br>`nl`<br>`pl`<br>`pt`<br>`sk`<br>`sl`<br>`sv` | `cs`, `da`, `de`, `el`, `en`, `es`, `et`, `fi`, `fr`, `hu`, `it`, `li`, `lt`, `lv`, `mt`, `nl`, `pl`, `pt`, `sk`, `sl` or `sv`
